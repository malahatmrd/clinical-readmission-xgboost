...
def download_dataset() -> tuple[Path, Path, Path]:
    ...
    if identifiers is None:
        raise ValueError("Dataset did not return identifier columns.")

    if features is None:
        raise ValueError("Dataset did not return feature columns.")

    if target is None:
        raise ValueError("Dataset did not return target columns.")

    identifiers_path = RAW_DATA_DIR / "identifiers.csv"
    features_path = RAW_DATA_DIR / "features.csv"
    target_path = RAW_DATA_DIR / "target.csv"

    identifiers.to_csv(identifiers_path, index=False)
