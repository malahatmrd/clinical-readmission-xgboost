...
The primary cohort retains the first observed encounter for each patient
and subsequently excludes terminal or hospice discharge dispositions.
...
The sensitivity cohort excludes terminal or hospice encounters before
selecting the first eligible observed encounter for each patient.
...
The robustness cohort retains all non-terminal eligible encounters.
...
Repeated encounters are retained in this cohort, so all subsequent
evaluation using this cohort must use patient-grouped splitting.

See `docs/cohort_definition.md` for the complete cohort methodology,
eligibility rules, reproducibility audit, and leakage-control rationale.